# Playerok Monitor 2.3.18 · Встроенные обновления Android

- В настройках появилась кнопка «Проверить обновления приложения». Новая версия проверяется, скачивается и передаётся штатному установщику Android прямо из Playerok Monitor.
- Приложение проверяет контрольную сумму, package id, рост versionCode и криптографическую подпись APK до открытия установки. Подменённый или повреждённый файл будет отклонён.
- Тихая автоматическая проверка выполняется не чаще двух раз в сутки и показывает одну подсказку на версию — без фоновой нагрузки на VPS и без повторяющегося спама.
- Начиная с 2.3.18 все Android-релизы подписываются одним постоянным закрытым ключом из GitHub Actions Secrets. Это делает последующие обновления установляемыми поверх приложения с сохранением Pairing URL и настроек.
- Из-за временных CI-ключей старых APK переход с 2.3.17 на 2.3.18 требует последней ручной переустановки. После установки 2.3.18 следующие обновления будут доступны внутри приложения.

## Included from 2.3.17
